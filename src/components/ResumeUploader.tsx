...
const ResumeUploader: React.FC<ResumeUploaderProps> = ({
  ...
  const uploadResume = async () => {
    ...
    try {
      ...
      const response = await fetch("http://127.0.0.1:5000/api/parse-resume", {
        method: "POST",
        body: formData,
      });
      ...
      toast({
        title: "Upload failed",
        description:
          "There was an error processing your resume. Make sure the backend server is running.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Card className="p-6 w-full max-w-md mx-auto bg-gradient-to-br from-zinc-900 to-black/60 border border-zinc-800/50 shadow-lg">
      ...
      >
        ...
        >
          <Upload
            className={`h-12 w-12 mb-3 transition-colors duration-300 ${
